...
def search_measurements(measurements):
    ...
    search_term = get_date_input(
        "Enter the date of measurement (dd/mm/yyyy): ")
    try:
        with open(measurements, "r") as open_measurements:
            reader = csv.reader(open_measurements)
            matching_rows = [
                row for row in reader if search_term.strftime("%d/%m/%Y") == row[0]]
            if len(matching_rows) == 0:
                print(
                    f"No measurements found with date {search_term.strftime('%d/%m/%Y')}.")
            else:
                table = PrettyTable()
